...
def data_result_dict(data,arguments):
    ...
    if type(data_result)==list:
        return data_result
    elif type(data_result)==dict:
        if "___" in data:
...
def apiRequest(api,arguments=None,reset_data=None):
    ...
    if api.data:
        #data请求类型的参数格式
        if request_type == "params" or request_type == "data":
        ...
        #json请求类型的参数格式
            ...
            data = data_result_dict(api.data,arguments)
                    # data_list = json.loads(api.data, encoding='utf-8')
                    # if type(data_list)==list:
                    #     data = data_list
                    # elif type(data_list)==dict:
                    #     if "___" in api.data:
                    #         data = data_function(data_list)
                    #     if "{{" in api.data:
                    #         data = data_argument(data_list, arguments)
                    #     else:
                    #         data = data_list
